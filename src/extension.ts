...
/**
 * Shows a status bar message that auto-dismisses after 3 seconds.
 * Used for non-critical startup notifications that shouldn't require user dismissal.
 */
    ...
    if (!cliStatus.isInstalled) {
        ...
        if (lastVersion && semver.gt(currentVersion, lastVersion)) {
            ...
            const message = migrated
                ? `Falcon Cloud Security: CLI upgraded to v${currentVersion}. Config migrated automatically. ✅`
                : `Falcon Cloud Security: CLI upgraded to v${currentVersion}. ✅`;
            showTransientMessage(message);
        } else {
            showTransientMessage('FCS CLI Plugin is ready. ✅');
        }
